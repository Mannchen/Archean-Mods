...
#INFO text "send_p" "100" "Output Power (MW)"
#INFO text "send_v" "500" "Output Voltage"
#INFO text "receive_w" "0" "Consume Power (MW)"
...
tick
	...
	if $new_info_send_power != $info_send_power
		if isnumeric($new_info_send_power)
			$g_send_power = $new_info_send_power * 1000000
		else
			set_info("send_p", $info_send_power / 1000000)
	...
	if $new_info_receive_power != $info_receive_power
		if isnumeric($new_info_receive_power)
			$g_receive_power = $new_info_receive_power * 1000000
		else
			set_info("receive_w", $info_receive_power / 1000000)
	...
	if $g_enable && $g_send_power > 0 && $g_send_volts > 0
		$power_pushed = push_power("hv", $g_send_volts, $g_send_power)

	var $power_pulled = 0
	if $g_enable && $g_receive_power > 0
		$power_pulled = pull_power("hv", 0, $g_receive_power)
